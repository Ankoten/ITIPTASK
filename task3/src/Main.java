...
public class Main {
    ...
    public static boolean doesBlockFit(int a, int b, int c, int w, int h) {
        int[] sides = {a, b, c};
        if (sides[0] <= w && sides[1] <= h) {
            return true;
        } else if (sides[1] <= h && sides[2] <= w) {
            return true;
        }  else if (sides[0] <= h && sides[2] <= w) {
            return true;
        }else {
            return false;
        }
    ...
    public static boolean numCheck(int a){
        ...
        Integer[] numAr = num.toArray(new Integer[0]);
    ...
    public static List <String> salesData(String[][] data){
        ...
        int max_l = data[0].length;
        for(int i = 1; i < data.length; i++){
            if(max_l < data[i].length){
                max_l = data[i].length;
            }
        }
        for(int i = 0; i < data.length; i++){
            if(data[i].length == max_l){
                result.add(data[i][0]);
            }
        }
        return result;
    ...
    public static  String commonVovel(String str){
        ...
        for(int i = 0; i < col.length; i++){
            if(col[i] == Arrays.stream(col).max().getAsInt()){
                result +=  vow.charAt(i);
            }
        }
        return result;
